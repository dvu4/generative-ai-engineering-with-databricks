...
# MAGIC
# MAGIC ![marketplace_home](../Includes/images/marketplace_home.png)
# MAGIC
...
# MAGIC
# MAGIC ![bright_data_amazon_reviews_dataset_overview](../Includes/images/bright_data_amazon_reviews_dataset_overview.png)
# MAGIC
...
# MAGIC %md
# MAGIC ## Rearc Personal Income | FRED
# MAGIC
# MAGIC In the next example, we'll look at a Personal Income dataset from Rearc.
# MAGIC
# MAGIC ![rearc_personal_income_fred_overview](../Includes/images/rearc_personal_income_fred_overview.png)
# MAGIC
# MAGIC
...
rearc_personal_income_fred = spark.sql("SELECT * FROM  rearc_personal_income_fred.fs_fred.fred_pi")
rearc_personal_income_fred.limit(10).display()

# COMMAND ----------

rearc_personal_income_fred_schema = spark.sql("SELECT * FROM rearc_personal_income_fred.information_schema.columns")
display(rearc_personal_income_fred_schema)
...
yahoo_finance_business_dataset = spark.sql("SELECT * FROM  bright_data_yahoo_finance_business_dataset.datasets.yahoo_finance_business")
yahoo_finance_business_dataset.display()
yahoo_finance_business_dataset.groupBy("ENTITY_TYPE").count().display()
yahoo_finance_business_dataset.where(F.col("STOCK_TICKER").contains("ROKU")).display()

# COMMAND ----------

yahoo_finance_business_schema = spark.sql(
    "SELECT * FROM bright_data_yahoo_finance_business_dataset.information_schema.columns"
)
display(yahoo_finance_business_schema)
