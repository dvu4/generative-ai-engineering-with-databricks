...
# MAGIC %pip install -qqqq -U mlflow==3.4.0 llama-index==0.14.5 PyPDF2==3.0.0
...
def file_description(path):
    ...
    df_files = spark.createDataFrame([
        ...
        "path": f.path,
        "size": f.size,
        "modification_time": f.modificationTime
        }
        for f in files
    ])
    from pyspark.sql.functions import from_unixtime, date_format
    df_files = df_files.withColumn(
    "chicago_modification_date",
    date_format(from_unixtime(df_files.modification_time / 1000), "yyyy-MM-dd HH:mm:ss")
    )
    df = df_files.select("file_name", "path", "size", "chicago_modification_date")
    display(df)
    return df
...
dataset_paths = [
    getattr(DA.paths.datasets, k)
    for k in vars(DA.paths.datasets)
]
# print(dataset_paths)
# print(dataset_paths[0])


dataset_values = [v for k, v in dataset_paths[0].items() if k.startswith("datasets.")]
# print(dataset_values)

# for p in dataset_values:
#     # print(p)
#     volumes = spark.sql(f"SHOW VOLUMES IN {p}")
#     print(f"Volumnes in {p} : ")
#     display(volumes)
...
for p in dataset_values:
    volumes = spark.sql(f"SHOW VOLUMES IN {p}")
    display(volumes)
    # Collect all volume names from the DataFrame
    volume_names = [row['volume_name'] for row in volumes.collect()]
    for volume_name in volume_names:
        articles_path = f"/Volumes/{p.replace('.', '/')}/{volume_name}/"
        print(f"Path: {articles_path}")
        file_description(articles_path)

# COMMAND ----------

articles_path = f"{DA.paths.datasets.arxiv}/arxiv-articles/"
articles_path = f"{DA.paths.datasets.arxiv}/evaluation/"
articles_path = f"{DA.paths.datasets.arxiv}/inference-sample/"
file_description(articles_path)

# COMMAND ----------

print(f"Dataset Location:  {DA.paths.datasets}")
print(f"arxiv Location :  {DA.paths.datasets.arxiv}")
# print(f"dais Location :  {DA.paths.datasets.dais}")
# print(f"news Location :  {DA.paths.datasets.news}")
# print(f"docs Location :  {DA.paths.datasets.docs}")
articles_path = f"{DA.paths.datasets.arxiv}/arxiv-articles/"
# articles_path = f"{DA.paths.datasets.dais}/dais_delta"
file_description(articles_path)
